// components/GraphicsClient.tsx (Client-Side Component)
'use client';
...
export default function GraphicsClient(): JSX.Element {
            ...
            <div className="embla__container flex transition-transform duration-[700ms] ease-in-out">
              {services.map((service, index) => (
                <div key={index} className="embla__slide flex-none w-full">
                  <div className={`flex flex-col md:flex-row items-stretch ${index % 2 === 0 ? 'md:flex-row-reverse' : 'md:flex-row'}`}>
                    <div className="flex-1 bg-gray-900 text-white flex items-center justify-center p-8">
                      <div>
                        <h3 className="text-4xl md:text-5xl font-semibold">{service.title}</h3>
                        <p className="mt-4 whitespace-pre-line text-lg md:text-xl">{service.description}</p>
                      </div>
                    </div>
